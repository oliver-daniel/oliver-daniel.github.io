    ...
    return function (t) {
        ...
        t.exports = class {
            ...
            onScroll() {
                ...
                e && document.dispatchEvent(new CustomEvent('scroll-in', {
                    detail: {
                        href: e.attributes.href.value
                    }
                }));
                (window.pageYOffset <= 5) && (this.removeCurrentActive({ignore: document.getElementById('home')}), document.dispatchEvent(new CustomEvent('scroll-in', {
                    detail: {
                        href: '#home'
                    }
                })))
            ...
            setActive(t) {
                if (!t.classList.contains(this.options.activeClass)) {
                    this.options.activeClass.trim().split(" ").forEach(e => t.classList.add(e))
                }
            }
            removeCurrentActive(t = {
                ignore: null
            }) {
                const {
                    hrefAttribute: e,
                ...
                this.menuList.querySelectorAll(`${o}:not([${e}="${t.ignore.getAttribute(e)}"])`).forEach(t => {
                    this.options.activeClass.trim().split(" ").forEach(e => t.classList.remove(e))
                })
            }
        }
    }])
